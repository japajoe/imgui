...
typedef bool (*ProcessCommandCallback)(const char* command);
...
struct ConsoleApp
{
    ...
    void Dispose()
    {
        if(Items.Size == 0 && History.Size == 0)
            return;
	...
	void Draw(const char* title, bool* p_open, const ImVec2& position, const ImVec2& size)
	{
		BeginNoWindowFlags();
		ImGui::SetNextWindowPos(position);
		ImGui::SetNextWindowSize(size);
		
		if (!ImGui::Begin(title, p_open, noWindowFlags))
		{
			ImGui::End();
			return;
		}
		...
		if(ImGui::IsWindowFocused() && !ImGui::IsAnyItemActive() && !ImGui::IsMouseClicked(0) && reclaim_focus)
